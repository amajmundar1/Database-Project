...
*/
   ...
   public static void addCustomer(DBProject esql){
	...
	try {
		ResultSet c = esql.executeQuery("SELECT MAX(customerID) AS max_id FROM customer");
		int cID = -1;
		if (c.next()) {
   ...
   public static void addRoom(DBProject esql){
	...
	try{
		...
		ResultSet res = esql.executeQuery(getHID);
		if (!res.isBeforeFirst()) {
			System.out.print("\tHotel ID Not Found\n");
		} else {
			System.out.print("\tEnter the Room Number: ");
			String roomNum = in.readLine();
			ResultSet res1 = esql.executeQuery("Select * FROM Room WHERE hotelID = " + hID + " AND roomNo = " + roomNum);
			if (res1.next()) {
   ...
   public static void addMaintenanceCompany(DBProject esql){
	...
	try {
		...
		ResultSet res = esql.executeQuery(check);
		if (res.next()) {
			...
			ResultSet c = esql.executeQuery("SELECT MAX(cmpID) AS max_id FROM MaintenanceCompany");
                	int cID = -1;
			if (c.next()) {
				cID = c.getInt("max_id") + 1;
			}
   ...
   public static void addRepair(DBProject esql){
	...
	try {
		...
		System.out.print("\tEnter the Room Number: ");
		String roomNum = in.readLine();
		ResultSet res = esql.executeQuery("Select * FROM Room WHERE hotelID = " + hID + " AND roomNo = " + roomNum);
		if (res.next()) {
			System.out.print("\tEnter the Maintenace Company ID: ");
			String maintID = in.readLine();
			ResultSet res1 = esql.executeQuery("SELECT * FROM MaintenanceCompany WHERE cmpID = " + maintID + " AND isCertified");
			if (res1.next()) {
				ResultSet c = esql.executeQuery("SELECT MAX(rID) AS max_id FROM Repair");
                        	int rID = -1;
                        	if (c.next()) {
   ...
   public static void bookRoom(DBProject esql){
	...
	try {
		...
		if (res.next()) {
			...
			String Lname = in.readLine();
			ResultSet getCust = esql.executeQuery("SELECT COUNT(*) AS rowcount FROM customer WHERE fname = \'" + Fname + "\' AND lname = \'" + Lname + "\'");
			if (getCust.next()) {
				int count = getCust.getInt("rowcount");
				String cID = "";
				ResultSet getCID = esql.executeQuery("SELECT * FROM customer WHERE fname = \'" + Fname + "\' AND lname = \'" + Lname + "\'");
				if (count == 1) {
					getCID.next();
				...
				ResultSet verify = esql.executeQuery("SELECT * FROM customer WHERE customerID = " + cID);
				if (verify.next()) {
					System.out.print("\tEnter the Date you with to book the room for in YYYY-MM-DD Format: ");
					String date = in.readLine();
					ResultSet checkDate = esql.executeQuery("SELECT * FROM Booking WHERE hotelID = " + hID + " AND roomNo = " + roomNum + " AND bookingDate = DATE \'" + date + "\'");
					if (!checkDate.next()) {
						ResultSet c = esql.executeQuery("SELECT MAX(bID) AS max_id FROM Booking");
                                		int bID = -1;
                                		if (c.next()) {
   ...
   public static void assignHouseCleaningToRoom(DBProject esql){
	...
	try {
		...
		System.out.print("\tEnter the Hotel ID: ");
		String hID = in.readLine();
		ResultSet staff = esql.executeQuery("SELECT * FROM Staff WHERE SSN = " + ssn + " AND employerID = " + hID + " AND role = \'HouseCleaning\'");
		String fname = "";
		if (staff.next()) {
			...
			ResultSet res = esql.executeQuery("Select * FROM Room WHERE hotelID = " + hID + " AND roomNo = " + roomNum);
                	if (res.next()) {
				 ResultSet c = esql.executeQuery("SELECT MAX(asgID) AS max_id FROM Assigned");
                                 int asgID = -1;
                                 if (c.next()) {
   ...
   public static void repairRequest(DBProject esql){
	...
	try {
		...
		String ssn = in.readLine();
		ResultSet checkHotelManager = esql.executeQuery("SELECT * FROM Hotel WHERE hotelID = " + hID + " AND manager = " + ssn);
		if (checkHotelManager.next()) {
			System.out.print("\tEnter the Room Number: ");
			String roomNum = in.readLine();
			System.out.print("\tEnter the Repair ID: ");
			String repID = in.readLine();
			ResultSet checkRepair = esql.executeQuery("SELECT * FROM Repair WHERE rID =" + repID + " AND hotelID = " + hID + " AND roomNo = " + roomNum);
			if (checkRepair.next()) {
				ResultSet c = esql.executeQuery("SELECT MAX(reqID) AS max_id FROM Request");
                		int rID = -1;
                		if (c.next()) {
                        		rID = c.getInt("max_id") + 1;
                		}
